...
$groups = ["202401_MD_AFIV06_7", "202401_MD_AFIV04_10","202401_MD_AFIV04_12"]; // Array de grupos
$fileCode = 1;
...
// Array para almacenar el recuento de grupos
$groupCount = array_fill_keys($groups, 0);

// Contar la frecuencia de cada grupo en el CSV
foreach ($csvData as $fila) {
    $grupoCSV = $fila[0];
    if (in_array($grupoCSV, $groups) && $fila[1] === $fileCodeStr) {
        $groupCount[$grupoCSV]++;
    }
}
...
if (empty($groups)) {
    ...
    ¡Vaya!, no perteneces a ningun grupo.
  </div></div>";
} else {
...
<p>Aquí estarán tus actividades de consolidación para los siguientes grupos. </p>
    ...
    foreach ($groupCount as $group => $count) {
        ...
        if ($count > 1) {
            ...
            <p>hay mas de una actividad para este grupo: $group</p>
        </div>";
        } else {
